...
# Define todos los objetivos y sus propiedades para hacerlo más dinámico
# 'type': 'bool' (checkbox), 'float' (número decimal), 'int' (número entero)
# 'section': 'daily', 'health', 'projects'
APP_OBJECTIVES_CONFIG = {
    # Objetivos Diarios
    'Entrenamiento_Hecho': {'display': '✅ Entrenamiento Hecho', 'type': 'bool', 'section': 'daily'},
    'Entrenamiento_Minutos': {'display': 'Minutos de Entrenamiento', 'type': 'float', 'section': 'daily', 'step': 15.0, 'goal_monthly': 900}, # Meta: 15 entrenamientos * 60 min
    'Comida Saludable': {'display': '✅ Comida Saludable', 'type': 'bool', 'section': 'daily'},
    'Agua_Litros': {'display': 'Litros de Agua', 'type': 'float', 'section': 'daily', 'step': 0.5, 'goal_daily_avg': 2.0}, # Meta: 2 litros/día
    'Horas Extra': {'display': 'Horas Extra', 'type': 'float', 'section': 'daily', 'step': 0.5, 'goal_monthly': 40}, # Meta: 40 horas/mes
    'Meditacion_Minutos': {'display': 'Minutos de Meditación', 'type': 'float', 'section': 'daily', 'step': 5.0, 'goal_daily_avg': 10.0}, # Meta: 10 min/día
    'Lectura_Paginas': {'display': 'Páginas Leídas', 'type': 'int', 'section': 'daily', 'step': 10, 'goal_monthly': 300}, # Meta: 300 páginas/mes

    # Objetivos de Salud (para el último registro del mes)
...
if os.path.exists(DATA_FILE):
    ...
    for col_name in APP_COLUMNS_NAMES:
        ...
        if not config: # Si la columna no está en la configuración (ej. columna vieja)
            continue # Ignorar o manejar según se prefiera, aquí la ignoramos
        ...
        if col_name not in df.columns:
            ...
            if col_type == 'bool':
                ...
                if col_name in ['App ingresos y salidas', 'App progreso personal']: # Default para proyectos
                    df[col_name] = 'Pendiente'
                else:
                    df[col_name] = '' # Default vacío para otros strings
        else:
            ...
            # 'str' ya está bien si se carga como string
    ...
    # Ajustar valores iniciales para booleanos, floats, ints y strings por defecto
    for col_name in APP_COLUMNS_NAMES:
        if col_name == 'Fecha':
            continue # La fecha se añadirá en la primera fila
        config = APP_OBJECTIVES_CONFIG.get(col_name)
        if config:
            col_type = config['type']
            if col_type == 'bool':
                ...
                if col_name in ['App ingresos y salidas', 'App progreso personal']:
                    df[col_name] = 'Pendiente'
                else:
                    df[col_name] = ''
...
with tab1:
    ...
    # Mostrar dinámicamente los objetivos diarios de la configuración
    ...
    for col_name, config in APP_OBJECTIVES_CONFIG.items():
        if config['section'] == 'daily':
            ...
            if config['type'] == 'bool':
                ...
                df.loc[row_index, col_name] = new_value
            daily_columns_for_display.append(col_name) # Para mostrar en la tabla de 7 días
...
with tab4:
    ...
    if not df_current_month.empty:
        ...
        # --- Cálculo de Progreso por Categoría (dinámico) ---
        category_progress = {}
        total_weight = 0

        # Objetivos Diarios Cuantificables
        ...
        for obj_name in daily_quant_objectives:
            ...
            current_value = df_current_month[obj_name].sum() # Suma para mensual
            
            if goal_monthly: # Si tiene una meta mensual (ej. Horas Extra, Lectura, Entrenamiento)
            ...
            elif goal_daily_avg: # Si tiene una meta diaria promedio (ej. Agua, Meditación)
                ...
                progress = 0 # No hay meta definida
            
            category_progress[obj_name] = progress
            
        # Objetivos Diarios Booleanos (porcentaje de días que se hizo)
        bool_daily_objectives = ['Entrenamiento_Hecho', 'Comida Saludable']
        for obj_name in bool_daily_objectives:
            config = APP_OBJECTIVES_CONFIG[obj_name]
            progress = df_current_month[obj_name].mean() if total_days_logged > 0 else 0 # Porcentaje de días True
            st.write(f"- **{config['display']} (promedio días):** {progress:.1%}")
            category_progress[obj_name] = progress
        
        # Objetivos de Salud (desde el último registro del mes)
        salud_completados = 0
        ...
        # Proyectos (desde el último registro del mes)
        ...
        # --- Ponderación y Progreso Total (Ajusta estos pesos a tu gusto) ---
        # Puedes añadir los pesos directamente en la configuración de cada objetivo si quieres más granularidad
        # O definirlos aquí para cada categoría general
        pesos = {
            'Entrenamiento_Minutos': 0.15,
            'Entrenamiento_Hecho': 0.05, # Puede tener menos peso si el min es lo principal
        ...
        total_weight = sum(pesos.values()) # Asegurar que los pesos sumen 1 o ajustar la división
        ...
        # Gráfico de Minutos de Entrenamiento
